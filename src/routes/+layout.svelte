<script lang="ts">
    ...
    let moreDropdownRefDesktop: HTMLDivElement | null = $state(null);
    let moreDropdownRefMobile: HTMLDivElement | null = $state(null);

    function handleClickOutsideMore(event: MouseEvent) {
        const targetNode = event.target as Node | null;
        if (!targetNode) return;
        const clickedInsideDesktop = moreDropdownRefDesktop?.contains(targetNode) ?? false;
        const clickedInsideMobile = moreDropdownRefMobile?.contains(targetNode) ?? false;
    ...
    onMount(() => {
        ...
        const handleClickOutside = (event: MouseEvent) => {
            const targetNode = event.target as Node | null;
            if (dropdownRef && targetNode && !dropdownRef.contains(targetNode)) {
				showDropdown = false;
			}
		};
        const handleClickOutside2 = (event: MouseEvent) => {
            const targetNode = event.target as Node | null;
            if (dropdownRef2 && targetNode && !dropdownRef2.contains(targetNode)) {
                showSearchBar = false;
			}
        };
...
<div class="font-jost overflow-hidden">
	<main class=" mx-auto">
        {#if page.url.pathname !== '/login' && page.url.pathname !== '/dashboard/superadmin' && page.url.pathname !== '/vendor/dashboard' && page.url.pathname !== '/signup' && page.url.pathname !== '/login/forgot-password'}
						...
						<li>
                            <select
                                ...
                                onchange={(e: Event) => {
                                    ...
                                    if (target) {
                                        user.preferred_currency = target.value;
                                    }
								}}
							>
			...
			<div class="flex max-w-6xl mx-auto justify-between items-center py-4 gap-4">
				...
				<a href="/" class="flex-shrink-0">
                    <img
                        src={(($settings.find((s) => s.name === 'logo')) && $settings.find((s) => s.name === 'logo')?.image)
                            ? pocketbase.files.getURL(
                                    $settings.find((s) => s.name === 'logo') as any,
                                    $settings.find((s) => s.name === 'logo')?.image as any
                                )
                            : ''}
						alt="logo"
				...
				>
                    <select
                        ...
                        onchange={(e: Event) => {
                            const target = e.target as HTMLSelectElement | null;
                            if (target) queryCategory = target.value;
                        }}
                    >
						<option>All</option>
				...
				{#if loginState}
					...
					<div class="hidden md:flex items-center gap-3 px-3">
                        ...
                        <a
                            href="/wishlist"
                            class="hidden md:block relative hover:shadow hover:rounded-full p-2"
                        >
							<Heart />
				...
				{:else}
					...
					<div class="hidden md:flex items-center gap-3 px-3">
						...
						<div class="relative">
							...
							>
                                <Heart fillColor="red" strokeColor="#000" />
								<span
			...
			<div class=" w-full font-semibold min-w-screen h-fit text-white bg-blue-700">
                <div class="flex flex-col lg:flex-row items-center max-w-6xl mx-auto justify-between bg-transparent py-2 px-0 my-2">
                    <!-- Left: Browse all categories -->
                        ...
                        >
                            {#each categories.slice(0, 3) as category}
                                <a
                                    ...
                                    class="font-semibold text-white hover:text-gray-700 transition-colors"
                                    >{category}</a
                                >
                            {/each}
                            <div class="relative "  bind:this={moreDropdownRefDesktop}>
                            <button
                                    class="font-semibold  text-white px-4 py-2  cursor-pointer  "
                                onclick={(e) => { e.stopPropagation(); showMoreDropdown = !showMoreDropdown; }}
                                >
                                    More...
                                </button>
                                {#if showMoreDropdown}
                                    <div
                                        class="absolute left-0 mt-2 w-48 bg-white text-black rounded shadow-lg z-50 animate-fade-in"
                                    >
                                        {#each categories.slice(3) as category}
                                            <a
                                                href={`/products/category/${encodeURIComponent(category.toLowerCase())}`}
                                                class="block px-4 py-2 hover:bg-gray-100"
                                                onclick={(e) => { e.stopPropagation(); showMoreDropdown = false; }}>{category}</a>
                                        {/each}
                                    </div>
                                {/if}
                            </div>
                            <!-- <a
                                href="/about"
                                class="font-semibold text-blue-700 hover:text-blue-900 transition-colors">About</a
                            >
                            <a
                                href="/contact"
                                class="font-semibold text-blue-700 hover:text-blue-900 transition-colors">Contact</a
                            >
                            <a
                                href="/faq"
                                class="font-semibold text-blue-700 hover:text-blue-900 transition-colors">FAQ</a
                            > -->
					</div>

                        <!-- Mobile: Compact category bar with scroll -->
                        <div class="lg:hidden w-full bg-white rounded-t-lg shadow-md py-2 px-2">
                            <div class="flex items-center justify-between">
                                <!-- Scrollable categories -->
                                <div class="flex-1  left-0 right-0 z-100 scrollbar-hide">
                                    <div class="flex space-x-4 px-2 min-w-max">
                                        {#each categories.slice(0, 2) as category}
                                            <a
                                                href={`/products/category/${encodeURIComponent(category.toLowerCase())}`}
                                                class="font-semibold text-blue-700 hover:text-blue-900 transition-colors whitespace-nowrap text-sm"
                                                >{category}</a
                                            >
                                        {/each}
                                        <div class="relative" bind:this={moreDropdownRefMobile}>
                                            <button
                                                class="font-semibold bg-blue-600 text-white px-3 py-1 rounded text-sm hover:bg-blue-700 transition-colors shadow"
                                                onclick={(e) => { e.stopPropagation(); showMoreDropdown = !showMoreDropdown; }}
                                            >
                                                More
                                            </button>
                                            {#if showMoreDropdown}
                                                <div
                                                    class="absolute left-0 mt-2 w-40 bg-white text-black rounded shadow-lg z-50 animate-fade-in max-h-60 overflow-y-auto"
                                                >
                                                    ...
                                                    <a
                                                        href="/about"
                                                        class="block px-3 py-2 hover:bg-gray-100 text-sm"
                                                        onclick={(e) => { e.stopPropagation(); showMoreDropdown = false; }}>About</a>
                                                    <a
                                                        href="/contact"
                                                        class="block px-3 py-2 hover:bg-gray-100 text-sm"
                                                        onclick={(e) => { e.stopPropagation(); showMoreDropdown = false; }}>Contact</a>
                                                    <a
                                                        href="/faq"
                                                        class="block px-3 py-2 hover:bg-gray-100 text-sm"
                                                        onclick={(e) => { e.stopPropagation(); showMoreDropdown = false; }}>FAQ</a>
                                                </div>
                                            {/if}
                                        </div>
                                    </div>
                    ...
                    <div class="flex-shrink-0 hidden lg:flex px-4  w-auto justify-end items-center">
                            <button
                            class="bg-black text-white  py-4  px-3 font-bold rounded"
                                onclick={(e) => { e.stopPropagation(); window.location.href = '/vendor/kys'; }}
                        >
                            Become a Seller &nbsp;{'>'}
                        </button>
					</div>
        ...
        {#if page.url.pathname !== '/login' && page.url.pathname !== '/dashboard/superadmin' && page.url.pathname !== '/vendor/dashboard' && page.url.pathname !== '/signup' && page.url.pathname !== '/login/forgot-password'}
            ...
            <nav
                class="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 shadow md:hidden flex justify-around items-center h-16"
            >
				<!-- Home -->
                <a
                    href="/"
                    class="flex flex-col items-center justify-center text-blue-700 hover:text-blue-900"
                >
					<Home />
                ...
                <button
                    type="button"
                    class="flex flex-col items-center justify-center text-blue-700 hover:text-blue-900 focus:outline-none"
                ...
                <a
                    href="/wishlist"
                    class="md:hidden text-center flex flex-col justify-center items-center relative hover:shadow hover:rounded-full p-2"
                >
					<Heart />
					{#if $wishlist && $wishlist.length > 0}
						<span
                ...
                <a
                    href="/cart"
                    class="relative hover:shadow text-center flex flex-col justify-center items-center hover:rounded-full p-2"
                >
					<ShoppingCart size={24} />
                    {#if $cart && $cart.items && $cart.items.length > 0}
						<span
...
<style>
    ...
    /* Hide scrollbar for mobile category scrolling */
    .scrollbar-hide {
